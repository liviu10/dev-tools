...
import type { PowerShellCheatSheetItem } from '../data/powershellCheatSheet';

type Tab = PowerShellCheatSheetItem['category'] | 'All';

const CheatSheetSection: React.FC<{ items: PowerShellCheatSheetItem[] }> = ({ items }) => (
...
const PowerShellCheatSheet: React.FC = () => {
    const [activeTab, setActiveTab] = useState<Tab>('All');
    const [searchQuery, setSearchQuery] = useState('');

    const categories = useMemo(() => {
        const uniqueCategories = [...new Set(powershellData.map(item => item.category))];
        const orderedCategories = [
            'Core Cmdlets', 'Object Manipulation', 'File System', 'Process & Service Management', 
            'Networking', 'System Information', 'Scripting Basics'
        ];
    ...
    const filteredItems = useMemo(() => {
        let items: PowerShellCheatSheetItem[];
        
        if (activeTab === 'All') {
            items = powershellData;
        } else {
            items = powershellData.filter(item => item.category === activeTab);
        }
    ...
    return (
        <div className="max-w-5xl mx-auto">
            <h2 className="text-3xl font-bold mb-6 text-white">PowerShell Cheat Sheet</h2>

            <div className="mb-6 relative">
                 <input
                    type="text"
                    placeholder="Search cmdlets, aliases..."
                    value={searchQuery}
                    ...
                    aria-label="Search PowerShell cmdlets"
                />
