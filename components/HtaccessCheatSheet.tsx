...
import type { HtaccessCheatSheetItem } from '../data/htaccessCheatSheet';

type Tab = HtaccessCheatSheetItem['category'] | 'All';

const CheatSheetSection: React.FC<{ items: HtaccessCheatSheetItem[] }> = ({ items }) => (
...
const HtaccessCheatSheet: React.FC = () => {
    const [activeTab, setActiveTab] = useState<Tab>('All');
    const [searchQuery, setSearchQuery] = useState('');

    const categories = useMemo(() => {
        const uniqueCategories = [...new Set(htaccessData.map(item => item.category))];
        // Custom order
    ...
    const filteredItems = useMemo(() => {
        let items: HtaccessCheatSheetItem[];
        
        if (activeTab === 'All') {
            items = htaccessData;
        } else {
            items = htaccessData.filter(item => item.category === activeTab);
        }
    ...
    return (
        <div className="max-w-5xl mx-auto">
            <h2 className="text-3xl font-bold mb-6 text-white">.htaccess Cheat Sheet</h2>

            <div className="mb-6 relative">
                 <input
                    type="text"
                    placeholder="Search directives, flags..."
                    value={searchQuery}
                    ...
                    aria-label="Search .htaccess directives"
                />
...
export default HtaccessCheatSheet;