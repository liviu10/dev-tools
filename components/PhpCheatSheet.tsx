...
import type { PhpCheatSheetItem } from '../data/phpCheatSheet';

type Tab = PhpCheatSheetItem['category'] | 'All';

const CheatSheetSection: React.FC<{ items: PhpCheatSheetItem[] }> = ({ items }) => (
...
const PhpCheatSheet: React.FC = () => {
    const [activeTab, setActiveTab] = useState<Tab>('All');
    const [searchQuery, setSearchQuery] = useState('');

    const categories = useMemo(() => {
        const uniqueCategories = [...new Set(phpData.map(item => item.category))];
        const orderedCategories = [
            'Syntax & Variables', 'Operators', 'Control Structures', 'Functions',
            'String Functions', 'Array Functions', 'Arrays', 'OOP', 'Date & Time', 'File & JSON Handling',
            'Database (PDO)', 'Security', 'Dependency Management (Composer)', 'Command Line (CLI)', 
            'Error Handling & Debugging', 'Superglobals', 'Newer Features'
        ];
    ...
    const filteredItems = useMemo(() => {
        let items: PhpCheatSheetItem[];
        
        if (activeTab === 'All') {
            items = phpData;
        } else {
            items = phpData.filter(item => item.category === activeTab);
        }
    ...
    return (
        <div className="max-w-5xl mx-auto">
            <h2 className="text-3xl font-bold mb-6 text-white">PHP Cheat Sheet</h2>

            <div className="mb-6 relative">
                 <input
                    type="text"
                    placeholder="Search syntax, functions, features..."
                    value={searchQuery}
