...
const JsValidator: React.FC = () => {
    ...
    const getBorderColor = () => {
        if (results.length === 0) return 'border-gray-600 focus-within:ring-indigo-500';
        ...
        return 'border-gray-600 focus-within:ring-indigo-500';
    };

    return (
        <div className="max-w-4xl mx-auto">
            <h2 className="text-3xl font-bold mb-6 text-white">JavaScript Sandbox</h2>
            <div className="bg-gray-800 p-4 sm:p-6 rounded-lg shadow-2xl border border-gray-700">
                <label htmlFor="input-js" className="block text-sm font-medium text-gray-300 mb-2">JavaScript Code</label>
                ...
                <div className="mt-6">
                    <h3 className="text-lg font-semibold text-gray-300 mb-2">Evaluation Output</h3>
                    <div className="bg-gray-900 border border-gray-700 rounded-lg max-h-60 overflow-y-auto">
                        {results.length === 0 ? (
                            ...
                            results.map((res, index) => (
                                <div key={index} className={`p-3 border-b border-gray-700 last:border-b-0 flex items-start space-x-3`}>
                                    {res.type === 'success' && (
                                    ...
                                    <div className="flex-1 text-sm">
                                        <div className="flex justify-between items-baseline">
                                            <p className={`font-bold ${
                                                res.type === 'success' ? 'text-green-400' :
                                        ...
                                        <p className={`whitespace-pre-wrap break-words ${
                                            res.type === 'log' ? 'text-blue-300 font-mono' : 'text-gray-200'
                                        }`}>{res.message}</p>
                                        {res.location && (
                                            <p className="text-gray-400 font-mono text-xs mt-1">{res.location}</p>
                                        )}
                                    </div>
                                </div>
                            ))
