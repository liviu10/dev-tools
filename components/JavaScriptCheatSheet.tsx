...
import type { JavaScriptCheatSheetItem } from '../data/javaScriptCheatSheet';

type Tab = JavaScriptCheatSheetItem['category'] | 'All';

const CheatSheetSection: React.FC<{ items: JavaScriptCheatSheetItem[] }> = ({ items }) => (
...
const JavaScriptCheatSheet: React.FC = () => {
    const [activeTab, setActiveTab] = useState<Tab>('All');
    const [searchQuery, setSearchQuery] = useState('');

    const categories = useMemo(() => {
        const uniqueCategories = [...new Set(jsData.map(item => item.category))];
        // Manually order categories for better UX
        const orderedCategories = [
            'Variables & Data Types', 'Operators', 'Control Flow', 'Functions', 
            'Arrays', 'Objects', 'ES6+ Features', 'Asynchronous JS', 'DOM Manipulation'
        ];
        const sorted = uniqueCategories.sort((a, b) => orderedCategories.indexOf(a) - orderedCategories.indexOf(b));
        return ['All', ...sorted] as Tab[];
    }, []);

    const filteredItems = useMemo(() => {
        let items: JavaScriptCheatSheetItem[];
        
        if (activeTab === 'All') {
            items = jsData;
        } else {
            items = jsData.filter(item => item.category === activeTab);
        }
    ...
    return (
        <div className="max-w-5xl mx-auto">
            <h2 className="text-3xl font-bold mb-6 text-white">JavaScript Cheat Sheet</h2>

            <div className="mb-6 relative">
                 <input
                    type="text"
                    placeholder="Search syntax, methods, features..."
                    value={searchQuery}
                    ...
                    aria-label="Search JavaScript syntax"
                />
