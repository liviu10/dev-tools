...
.page-container { display: grid; grid-template-columns: ${gridTemplateColumns}; grid-template-rows: ${gridTemplateRows}; grid-template-areas:
  ${gridTemplateAreas}; height: 100vh; gap: 1rem; padding: 1rem; }
.header { grid-area: header; } .sidebar { grid-area: sidebar; } .main-content { grid-area: main; } .footer { grid-area: footer; }
.header, .footer, .sidebar, .main-content { display: grid; background-color: #1f2937; border: 1px solid #374151; padding: 1rem; border-radius: 0.5rem; }
...
.header-item, .sidebar-item, .footer-item, .grid-item { display: flex; align-items: center; justify-content: center; min-height: 4rem; border-radius: 0.25rem; background-color: #374151; border: 1px solid #4b5563; padding: 0.5rem; color: #e5e7eb; overflow: hidden; }
/* Component Styles */
