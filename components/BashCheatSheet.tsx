...
import type { BashCheatSheetItem } from '../data/bashCheatSheet';

type Tab = BashCheatSheetItem['category'] | 'All';

const CheatSheetSection: React.FC<{ items: BashCheatSheetItem[] }> = ({ items }) => (
...
const BashCheatSheet: React.FC = () => {
    const [activeTab, setActiveTab] = useState<Tab>('All');
    const [searchQuery, setSearchQuery] = useState('');

    const categories = useMemo(() => {
        const uniqueCategories = [...new Set(bashData.map(item => item.category))];
        const orderedCategories = [
            'File & Directory Commands', 'Text Processing', 'System & Process Management', 'Permissions', 
            'Networking', 'Archive & Compression', 'Shell Scripting Basics', 'Shortcuts & History'
        ];
    ...
    const filteredItems = useMemo(() => {
        let items: BashCheatSheetItem[];
        
        if (activeTab === 'All') {
            items = bashData;
        } else {
            items = bashData.filter(item => item.category === activeTab);
        }
    ...
    return (
        <div className="max-w-5xl mx-auto">
            <h2 className="text-3xl font-bold mb-6 text-white">Bash Cheat Sheet</h2>
            ...
            <div className="mb-6 relative">
                 <input
                    ...
                    aria-label="Search Bash commands"
                />
