...
import type { GitCheatSheetItem } from '../data/gitCheatSheet';

type Tab = GitCheatSheetItem['category'] | 'All';

const CheatSheetSection: React.FC<{ items: GitCheatSheetItem[] }> = ({ items }) => (
...
const GitCheatSheet: React.FC = () => {
    ...
    const categories = useMemo(() => {
        const uniqueCategories = [...new Set(gitData.map(item => item.category))];
        const orderedCategories = [
            'Setup & Config', 'Starting a Project', 'Making Changes', 'Branching & Merging', 
            'Remote Repositories', 'History & Undoing Changes', 'Tagging', 'Advanced Git'
        ];
    ...
    const filteredItems = useMemo(() => {
        let items: GitCheatSheetItem[];
        
        if (activeTab === 'All') {
            items = gitData;
        } else {
            items = gitData.filter(item => item.category === activeTab);
        }
    ...
    return (
        <div className="max-w-5xl mx-auto">
            <h2 className="text-3xl font-bold mb-6 text-white">Git Cheat Sheet</h2>
