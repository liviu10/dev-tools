...
import type { CheatSheetItem } from '../data/htmlCheatSheet';

type Tab = CheatSheetItem['category'] | 'All' | 'Attributes';

const CheatSheetSection: React.FC<{ items: CheatSheetItem[] }> = ({ items }) => (
...
const HtmlCheatSheet: React.FC = () => {
    ...
    const categories = useMemo(() => {
        const tagCategories = [...new Set(allHtmlTags.map(tag => tag.category))].sort();
        return ['All', ...tagCategories, 'Attributes'] as Tab[];
    }, []);
    ...
    const filteredItems = useMemo(() => {
        ...
        if (activeTab === 'Attributes') {
            items = commonAttributes;
        } else if (activeTab === 'All') {
            items = allHtmlTags;
        } else {
            items = allHtmlTags.filter(tag => tag.category === activeTab);
        }
    ...
    return (
        <div className="max-w-5xl mx-auto">
            <h2 className="text-3xl font-bold mb-6 text-white">HTML Cheat Sheet</h2>

            <div className="mb-6 relative">
                 <input
                    type="text"
                    placeholder="Search tags and attributes..."
                    value={searchQuery}
