...
import type { CssCheatSheetItem } from '../data/cssCheatSheet';

type Tab = CssCheatSheetItem['category'] | 'All';

const CheatSheetSection: React.FC<{ items: CssCheatSheetItem[] }> = ({ items }) => (
...
const CssCheatSheet: React.FC = () => {
    const [activeTab, setActiveTab] = useState<Tab>('All');
    const [searchQuery, setSearchQuery] = useState('');

    const categories = useMemo(() => {
        const uniqueCategories = [...new Set(cssData.map(item => item.category))].sort();
        return ['All', ...uniqueCategories] as Tab[];
    }, []);

    const filteredItems = useMemo(() => {
        let items: CssCheatSheetItem[];
        
        if (activeTab === 'All') {
            items = cssData;
        } else {
            items = cssData.filter(item => item.category === activeTab);
        }
    ...
    return (
        <div className="max-w-5xl mx-auto">
            <h2 className="text-3xl font-bold mb-6 text-white">CSS Cheat Sheet</h2>

            <div className="mb-6 relative">
                 <input
                    type="text"
                    placeholder="Search properties, selectors..."
                    value={searchQuery}
