...
import type { DrfCheatSheetItem } from '../data/drfCheatSheet';

type Tab = DrfCheatSheetItem['category'] | 'All';

const CheatSheetSection: React.FC<{ items: DrfCheatSheetItem[] }> = ({ items }) => (
...
const DrfCheatSheet: React.FC = () => {
    const [activeTab, setActiveTab] = useState<Tab>('All');
    const [searchQuery, setSearchQuery] = useState('');

    const categories = useMemo(() => {
        const uniqueCategories = [...new Set(drfData.map(item => item.category))];
        const orderedCategories = [
            'Serializers', 'Views & ViewSets', 'Routing', 'Authentication & Permissions',
            'Pagination', 'Filtering, Ordering & Searching', 'Testing'
        ];
    ...
    const filteredItems = useMemo(() => {
        let items: DrfCheatSheetItem[];
        
        if (activeTab === 'All') {
            items = drfData;
        } else {
            items = drfData.filter(item => item.category === activeTab);
        }
    ...
    return (
        <div className="max-w-5xl mx-auto">
            <h2 className="text-3xl font-bold mb-6 text-white">Django Rest Framework Cheat Sheet</h2>

            <div className="mb-6 relative">
                 <input
                    type="text"
                    placeholder="Search serializers, views, routers..."
                    value={searchQuery}
