...
import type { LaravelCheatSheetItem } from '../data/laravelCheatSheet';

type Tab = LaravelCheatSheetItem['category'] | 'All';

const CheatSheetSection: React.FC<{ items: LaravelCheatSheetItem[] }> = ({ items }) => (
...
const LaravelCheatSheet: React.FC = () => {
    const [activeTab, setActiveTab] = useState<Tab>('All');
    const [searchQuery, setSearchQuery] = useState('');

    const categories = useMemo(() => {
        const uniqueCategories = [...new Set(laravelData.map(item => item.category))];
        const orderedCategories = [
            'Artisan Commands', 'Routing', 'Middleware', 'Controllers', 'Blade Templates', 
            'Eloquent ORM', 'Database & Migrations', 'Validation', 'Authentication',
            'Collections', 'File Storage', 'Queues', 'Task Scheduling', 'Testing'
        ];
    ...
    const filteredItems = useMemo(() => {
        let items: LaravelCheatSheetItem[];
        
        if (activeTab === 'All') {
            items = laravelData;
        } else {
            items = laravelData.filter(item => item.category === activeTab);
        }
    ...
    return (
        <div className="max-w-5xl mx-auto">
            <h2 className="text-3xl font-bold mb-6 text-white">Laravel Cheat Sheet</h2>

            <div className="mb-6 relative">
                 <input
                    type="text"
                    placeholder="Search Artisan, Blade, Eloquent..."
                    value={searchQuery}
                    ...
                    aria-label="Search Laravel syntax"
                />
